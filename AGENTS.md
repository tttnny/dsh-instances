...
- 打包分两种：.app 一律产出到 Downloads 目录（pnpm tauri build --bundles app，产物在 src-tauri target release bundle macos 下，拷过去覆盖；自测也只用这个，绝不碰本机 /Applications）。发版用的 dmg 在推送时才打，推到 GitHub release。日常开发优先 dev 验证。
