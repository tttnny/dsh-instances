...
- 打包分两种：agent 自测可自行 pnpm tauri build --bundles app 产出 .app（在 src-tauri target release bundle macos 下）自己安装验证，需要交付给人时拷到 Downloads 目录覆盖；发版用的 dmg 只在被明确要求时打，推到 GitHub release。日常开发优先 dev 验证。
